...
class ListAlertPoliciesActionTestCase(OpsGenieBaseActionTestCase):
    __test__ = True
    action_cls = ListAlertPoliciesAction

    def test_run_api_404(self):
        action, adapter = self._get_action_status_code(
            'GET',
            "mock://api.opsgenie.com/v2/policies/alert",
            status_code=404)
    ...
    def test_run_api_success(self):
        expected = self.load_json("list_alert_policies.json")

        action, adapter = self._get_mocked_action()
        adapter.register_uri('GET',
                             "mock://api.opsgenie.com/v2/policies/alert",
                             text=self.get_fixture_content("list_alert_policies.json"))
